/*
    FileName: party_ipm_rel_non_mastered_party_nb_agreement.sql
    Author: mma2156
    SUBJECT AREA : Party
    Table_Name :REL_NON_MASTERED_PARTY_NB_AGREEMENT
    SOURCE: ipm
    Teradata Source Code: 14                       
    Create Date:2022-04-19
        
    ===============================================================================================================
    Version/JIRA Story#     Created By     Last_Modified_Date   Description
    ---------------------------------------------------------------------------------------------------------------
    TERSUN-4212		               Party-Tier2         22/04           First Version Tier-2 
    ------------------------------------------------------------------------------------------------------------------
*/ 

/*COLLECT STATS ON SOURCE TABLE */
SELECT ANALYZE_STATISTICS ('PROD_NBR_VW_TERSUN.IPM_POL_VW');

DROP TABLE IF EXISTS VT_PRE_WORK;

CREATE LOCAL TEMPORARY TABLE VT_PRE_WORK ON COMMIT PRESERVE ROWS AS
SELECT * FROM EDW_WORK.PARTY_IPM_REL_NON_MASTERED_PARTY_NB_AGREEMENT WHERE 1<>1;


INSERT /*DIRECT*/ INTO VT_PRE_WORK(
DIM_AGREEMENT_NATURAL_KEY_HASH_UUID,
DIM_NON_MASTERED_PARTY_NATURAL_KEY_HASH_UUID,
REF_PARTY_ROLE_NATURAL_KEY_HASH_UUID,
BEGIN_DT,
BEGIN_DTM,
ROW_PROCESS_DTM,
AUDIT_ID,
LOGICAL_DELETE_IND,
CHECK_SUM,
CURRENT_ROW_IND,
END_DT,
END_DTM,
SOURCE_SYSTEM_ID,
RESTRICTED_ROW_IND,
UPDATE_AUDIT_ID,
SOURCE_DELETE_IND
)
SELECT
DIM_AGREEMENT_NATURAL_KEY_HASH_UUID,
UUID_GEN(LPAD(BTRIM(POL_NR),20,'0') || COALESCE (INSD_NM,'') || COALESCE (INSD_MID_NM,'') || COALESCE (INSD_LST_NM,'') ||'14')::UUID AS DIM_NON_MASTERED_PARTY_NATURAL_KEY_HASH_UUID,
REF_PARTY_ROLE_NATURAL_KEY_HASH_UUID,
BEGIN_DT,
BEGIN_DTM,
CURRENT_TIMESTAMP(6) AS  ROW_PROCESS_DTM,
:audit_id AS AUDIT_ID,
FALSE AS LOGICAL_DELETE_IND,
UUID_GEN(SOURCE_DELETE_IND)::uuid AS CHECK_SUM,
TRUE AS CURRENT_ROW_IND,
END_DT,
END_DTM,
SOURCE_SYSTEM_ID,
FALSE AS RESTRICTED_ROW_IND,
:audit_id AS UPDATE_AUDIT_ID,
SOURCE_DELETE_IND
FROM(
SELECT
POL_NR,
CLEAN_STRING(VOLTAGEACCESS(INSD_NM,'name')) AS INSD_NM,
CLEAN_STRING(VOLTAGEACCESS(INSD_MID_NM,'name')) AS INSD_MID_NM,
CLEAN_STRING(VOLTAGEACCESS(INSD_LST_NM,'name')) AS INSD_LST_NM,
UUID_GEN(CLEAN_STRING(ADMIN_SYS_CODE), CLEAN_STRING('IPM'), NULL, LPAD(BTRIM(POL_NR),20,'0'), NULL)::UUID AS DIM_AGREEMENT_NATURAL_KEY_HASH_UUID,
UUID_GEN('Insd')::uuid AS REF_PARTY_ROLE_NATURAL_KEY_HASH_UUID,
REC_STRT_DT::DATE AS BEGIN_DT,
REC_STRT_DT::TIMESTAMP(6) AS BEGIN_DTM,
REC_END_DT::DATE AS END_DT,
REC_END_DT::TIMESTAMP(6) AS END_DTM,
'343' AS SOURCE_SYSTEM_ID,
CASE WHEN CLEAN_STRING(SRC_DEL_IND)='Yes' THEN TRUE ELSE FALSE END  AS SOURCE_DELETE_IND  FROM
PROD_NBR_VW_TERSUN.IPM_POL_VW WHERE SRC_SYS_ID='14'
)Q_1 ;


COMMIT;

/* CALCULATE ROW_NUMBERS TO IDENTIFY HISTORY DATA  */
DROP TABLE IF EXISTS VT_ORDER_BY;

CREATE LOCAL TEMPORARY TABLE VT_ORDER_BY ON COMMIT PRESERVE ROWS AS 
SELECT *, ROW_NUMBER() OVER(PARTITION BY DIM_NON_MASTERED_PARTY_NATURAL_KEY_HASH_UUID, DIM_AGREEMENT_NATURAL_KEY_HASH_UUID, 
            REF_PARTY_ROLE_NATURAL_KEY_HASH_UUID ORDER BY END_DTM DESC, BEGIN_DTM DESC) AS RW_NUM 
FROM VT_PRE_WORK
ORDER BY DIM_NON_MASTERED_PARTY_NATURAL_KEY_HASH_UUID, 
DIM_AGREEMENT_NATURAL_KEY_HASH_UUID, 
REF_PARTY_ROLE_NATURAL_KEY_HASH_UUID,
END_DTM,
BEGIN_DTM;

/* TRUNCATE WORK TABLE */
TRUNCATE TABLE EDW_WORK.PARTY_IPM_REL_NON_MASTERED_PARTY_NB_AGREEMENT;

/* ASSIGNING PROPER BEGIN AND END_DATES FOR HISTORY DATA */

INSERT /*DIRECT*/ INTO EDW_WORK.PARTY_IPM_REL_NON_MASTERED_PARTY_NB_AGREEMENT
(
DIM_AGREEMENT_NATURAL_KEY_HASH_UUID,
DIM_NON_MASTERED_PARTY_NATURAL_KEY_HASH_UUID,
REF_PARTY_ROLE_NATURAL_KEY_HASH_UUID,
BEGIN_DT,
BEGIN_DTM,
ROW_PROCESS_DTM,
AUDIT_ID,
LOGICAL_DELETE_IND,
CHECK_SUM,
CURRENT_ROW_IND,
END_DT,
END_DTM,
SOURCE_SYSTEM_ID,
RESTRICTED_ROW_IND,
UPDATE_AUDIT_ID,
SOURCE_DELETE_IND
)
SELECT
A.DIM_AGREEMENT_NATURAL_KEY_HASH_UUID,
A.DIM_NON_MASTERED_PARTY_NATURAL_KEY_HASH_UUID,
A.REF_PARTY_ROLE_NATURAL_KEY_HASH_UUID,
A.BEGIN_DT,
A.BEGIN_DTM,
A.ROW_PROCESS_DTM,
A.AUDIT_ID,
A.LOGICAL_DELETE_IND,
A.CHECK_SUM,
CASE WHEN A.RW_NUM > B.RW_NUM AND B.RW_NUM = A.RW_NUM-1 AND A.CURRENT_ROW_IND=TRUE THEN FALSE ELSE A.CURRENT_ROW_IND END AS CURRENT_ROW_IND,
CASE WHEN A.RW_NUM>B.RW_NUM AND B.RW_NUM=A.RW_NUM-1 AND A.CURRENT_ROW_IND=TRUE THEN B.BEGIN_DT - INTERVAL '1' DAY 
         WHEN A.RW_NUM=1 AND A.CURRENT_ROW_IND=TRUE AND A.END_DT<>'9999-12-31' THEN '9999-12-31'::DATE
         ELSE A.END_DT END AS END_DT,
CASE WHEN A.RW_NUM>B.RW_NUM AND B.RW_NUM=A.RW_NUM-1 AND A.CURRENT_ROW_IND=TRUE THEN B.BEGIN_DTM - INTERVAL '1' SECOND 
         WHEN A.RW_NUM=1 AND A.CURRENT_ROW_IND=TRUE AND A.END_DT<>'9999-12-31' THEN '9999-12-31'::TIMESTAMP(6)
         ELSE A.END_DTM END AS END_DTM,
A.SOURCE_SYSTEM_ID,
A.RESTRICTED_ROW_IND,
A.UPDATE_AUDIT_ID,
A.SOURCE_DELETE_IND
FROM VT_ORDER_BY A
LEFT JOIN 
VT_ORDER_BY B
ON 
A.DIM_NON_MASTERED_PARTY_NATURAL_KEY_HASH_UUID=B.DIM_NON_MASTERED_PARTY_NATURAL_KEY_HASH_UUID AND 
A.DIM_AGREEMENT_NATURAL_KEY_HASH_UUID=B.DIM_AGREEMENT_NATURAL_KEY_HASH_UUID AND 
A.REF_PARTY_ROLE_NATURAL_KEY_HASH_UUID =B.REF_PARTY_ROLE_NATURAL_KEY_HASH_UUID AND 
A.RW_NUM = B.RW_NUM+1;

COMMIT;

/* DELETE ALREADY EXISTING DATA AS ITS FRESH LOAD */
DELETE FROM EDW.REL_NON_MASTERED_PARTY_NB_AGREEMENT
WHERE SOURCE_SYSTEM_ID IN ('14','343');

COMMIT;

/* INSERT INTO TARGET TABLE*/

INSERT /*DIRECT*/ INTO EDW.REL_NON_MASTERED_PARTY_NB_AGREEMENT
(
DIM_AGREEMENT_NATURAL_KEY_HASH_UUID,
DIM_NON_MASTERED_PARTY_NATURAL_KEY_HASH_UUID,
REF_PARTY_ROLE_NATURAL_KEY_HASH_UUID,
BEGIN_DT,
BEGIN_DTM,
ROW_PROCESS_DTM,
AUDIT_ID,
LOGICAL_DELETE_IND,
CHECK_SUM,
CURRENT_ROW_IND,
END_DT,
END_DTM,
SOURCE_SYSTEM_ID,
RESTRICTED_ROW_IND,
UPDATE_AUDIT_ID,
SOURCE_DELETE_IND
)
SELECT
DIM_AGREEMENT_NATURAL_KEY_HASH_UUID,
DIM_NON_MASTERED_PARTY_NATURAL_KEY_HASH_UUID,
REF_PARTY_ROLE_NATURAL_KEY_HASH_UUID,
BEGIN_DT,
BEGIN_DTM,
ROW_PROCESS_DTM,
AUDIT_ID,
LOGICAL_DELETE_IND,
CHECK_SUM,
CURRENT_ROW_IND,
END_DT,
END_DTM,
SOURCE_SYSTEM_ID,
RESTRICTED_ROW_IND,
UPDATE_AUDIT_ID,
SOURCE_DELETE_IND
FROM 
EDW_WORK.PARTY_IPM_REL_NON_MASTERED_PARTY_NB_AGREEMENT;

COMMIT;

/* COLLECT STATS FOR PERFORMANCE */
SELECT ANALYZE_STATISTICS ('EDW.REL_NON_MASTERED_PARTY_NB_AGREEMENT');