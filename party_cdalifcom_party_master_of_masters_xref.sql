/*
    FileName: party_cdalifcom_party_master_of_masters_xref.sql
    Author: MM14803
    Subject Area : Party
    Source: CDA LIF COM
    ...
    ---------------------------------------------------------------------------------------------------------------
    TERSUN-3426             Party-Tier2    08/25                Initial version      
    ------------------------------------------------------------------------------------------------------------------
*/
...
(
	...
	CASE WHEN (BEN_FRST_NM,
				...
				HLDG_KEY_SFX,
				BEN_ARGMT_TXT,
				BEN_ROW_CNTR_CD) IS NULL 
		THEN UUID_GEN(NULL)::UUID 
		ELSE UUID_GEN(BEN_FRST_NM,
					  BEN_MDL_NM,
					  BEN_LST_NM,
					  CARR_ADMIN_SYS_CD, 
					  HLDG_KEY_PFX,
					  ...
					  HLDG_KEY_SFX,
					  BEN_ARGMT_TXT,
					  BEN_ROW_CNTR_CD)::UUID END 					AS DIM_PARTY_NATURAL_KEY_HASH_UUID
	,CASE WHEN (BEN_FRST_NM,
				BEN_MDL_NM,
				BEN_LST_NM,
				CARR_ADMIN_SYS_CD,
		...
		ELSE UUID_GEN(BEN_FRST_NM,
					  ...
					  HLDG_KEY_SFX,
					  BEN_ARGMT_TXT,
					  BEN_ROW_CNTR_CD)::UUID END 					AS DIM_PRIOR_PARTY_NATURAL_KEY_HASH_UUID
	,COALESCE(BEN_FRST_NM,'')
	||COALESCE(BEN_MDL_NM,'')
	||COALESCE(BEN_LST_NM,'')
	...
	||COALESCE(SUBSTRING(BEN_ARGMT_TXT,1,100),'')
	...
	||COALESCE(HLDG_KEY_SFX,'')
	||COALESCE(SUBSTRING(BEN_ARGMT_TXT,1,100),'')
	||COALESCE(BEN_ROW_CNTR_CD,'') 									AS PARTY_PRIOR_ID  
	,SOR_PARTY_ID
	,PARTY_ID_TYPE_CDE 
	,BEGIN_DT  
	,BEGIN_DTM  
	...
	(
		...
		CLEAN_STRING(BEN_FRST_NM),
		CLEAN_STRING(BEN_MDL_NM),
		CLEAN_STRING(BEN_LST_NM),
