/*
    FileName: party_tpp_rel_agreement_nb_requirements.sql
    Author: MM69917	
    Subject Area : Party
    Source: TPP
    ...
    ---------------------------------------------------------------------------------------------------------------
    TERSUN-3702             Party-Tier2    07/14                Initial version      
    ------------------------------------------------------------------------------------------------------------------
*/
...
SELECT 
APPL_ID AS APPL_ID,
COALESCE(CLEAN_STRING(src.REQ_CD),'Unk') AS REQ_CD,
...
(
SELECT 
    CASE WHEN (CARR_ADMIN_SYS_CD, HLDG_KEY_PFX, HLDG_KEY, HLDG_KEY_SFX, CAS_ID) IS NULL 
         THEN UUID_GEN(NULL)::UUID 
         ELSE UUID_GEN(CARR_ADMIN_SYS_CD, 'Appl', HLDG_KEY_PFX, HLDG_KEY, HLDG_KEY_SFX, CAS_ID)::UUID 
...
SELECT 
    ...
    A.SOURCE_DELETE_IND
FROM VT_PRE_WORK A 
order by dim_agreement_natural_key_hash_uuid,ref_requirement_type_natural_key_hash_uuid,requirement_case_id;

commit;

DELETE FROM EDW.REL_AGREEMENT_NB_REQUIREMENT WHERE SOURCE_SYSTEM_ID IN ('347','32');
commit;
