...
SELECT * FROM EDW_STAGING.PARTY_DIPMS_DIM_NON_MASTERED_PARTY_PRE_WORK
WHERE 1<>1;
...
WHERE DIM_NON_MASTERED_PARTY_NATURAL_KEY_HASH_UUID NOT IN 
(
SELECT DISTINCT DIM_NON_MASTERED_PARTY_NATURAL_KEY_HASH_UUID
FROM VT_PRE_WORK
);
...
/* 
 * Code to handle history coming for a party in each delta: 
 * 1. Filter out incoming party uuid's whose latest check_sum of its history is already available in target
 * 2. Derive end_dt,end_dtm,current_row_ind values within each party UUID set
 * 3. Insert data into pre work table 
 */
 
CREATE LOCAL TEMPORARY TABLE VT_DELTA_UUIDS_RANK ON COMMIT PRESERVE ROWS AS  
SELECT 
DIM_NON_MASTERED_PARTY_NATURAL_KEY_HASH_UUID, 
CHECK_SUM,
ROW_NUMBER() OVER(PARTITION BY DIM_NON_MASTERED_PARTY_NATURAL_KEY_HASH_UUID ORDER BY LST_UPDT_DT DESC) RNK 
FROM VT_PRE_WORK; 

CREATE LOCAL TEMPORARY TABLE VT_UUIDS_WITH_UPDATES ON COMMIT PRESERVE ROWS AS  
SELECT STG.DIM_NON_MASTERED_PARTY_NATURAL_KEY_HASH_UUID
FROM VT_DELTA_UUIDS_RANK STG 
JOIN EDW.DIM_NON_MASTERED_PARTY TGT 
ON STG.DIM_NON_MASTERED_PARTY_NATURAL_KEY_HASH_UUID = TGT.DIM_NON_MASTERED_PARTY_NATURAL_KEY_HASH_UUID
AND STG.RNK=1
AND TGT.CURRENT_ROW_IND 
WHERE STG.CHECK_SUM <> TGT.CHECK_SUM; 

CREATE LOCAL TEMPORARY TABLE VT_PRE_WORK_TYPE2 ON COMMIT PRESERVE ROWS AS 
SELECT STG.*, ROW_NUMBER() OVER(PARTITION BY STG.DIM_NON_MASTERED_PARTY_NATURAL_KEY_HASH_UUID ORDER BY LST_UPDT_DT) AS RW_NUM
FROM VT_PRE_WORK STG 
LEFT JOIN EDW.DIM_NON_MASTERED_PARTY TGT 
ON STG.DIM_NON_MASTERED_PARTY_NATURAL_KEY_HASH_UUID = TGT.DIM_NON_MASTERED_PARTY_NATURAL_KEY_HASH_UUID
AND STG.CHECK_SUM = TGT.CHECK_SUM 
WHERE TGT.CHECK_SUM IS NULL
AND STG.DIM_NON_MASTERED_PARTY_NATURAL_KEY_HASH_UUID NOT IN 
(
SELECT DISTINCT DIM_NON_MASTERED_PARTY_NATURAL_KEY_HASH_UUID
FROM VT_UUIDS_WITH_UPDATES
);

INSERT INTO EDW_STAGING.PARTY_DIPMS_DIM_NON_MASTERED_PARTY_PRE_WORK(
DIM_NON_MASTERED_PARTY_NATURAL_KEY_HASH_UUID,
PARTY_ID,
FIRST_NM,
MIDDLE_NM,
...
SELECT 
T1.DIM_NON_MASTERED_PARTY_NATURAL_KEY_HASH_UUID,
T1.PARTY_ID,
T1.FIRST_NM,
T1.MIDDLE_NM,
T1.LAST_NM,
...
T1.BIRTH_DT,
T1.GENDER_CDE,
T1.SENSITIVE_PARTY_IND,
T1.SOURCE_GENDER_CDE,
T1.MARITAL_STATUS_CDE,
T1.SOURCE_MARITAL_STATUS_CDE, 
T1.GOVERNMENT_ID,
T1.SOURCE_LOGIN_ID,
T1.BEGIN_DT,
T1.BEGIN_DTM,
T1.ROW_PROCESS_DTM,
T1.AUDIT_ID,
T1.LOGICAL_DELETE_IND,
T1.CHECK_SUM,
CASE WHEN T1.RW_NUM<T2.RW_NUM AND T1.RW_NUM=T2.RW_NUM-1 THEN FALSE ELSE T1.CURRENT_ROW_IND END AS CURRENT_ROW_IND,
CASE WHEN T1.RW_NUM<T2.RW_NUM AND T1.RW_NUM=T2.RW_NUM-1 AND T1.END_DT > T2.BEGIN_DT - INTERVAL '1' DAY THEN T2.BEGIN_DT - INTERVAL '1' DAY 
ELSE T1.END_DT END AS END_DT,
CASE WHEN T1.RW_NUM<T2.RW_NUM AND T1.RW_NUM=T2.RW_NUM-1 AND T1.END_DTM > T2.BEGIN_DTM - INTERVAL '1' SECOND THEN T2.BEGIN_DTM - INTERVAL '1' SECOND
ELSE T1.END_DTM END AS END_DTM,
T1.SOURCE_SYSTEM_ID,
T1.RESTRICTED_ROW_IND,
T1.UPDATE_AUDIT_ID,
T1.SOURCE_DELETE_IND,
T1.PARTY_TYPE_CDE,
T1.GROUP_NBR,
T1.SUB_GROUP_NBR,
T1.GROUP_IPN_ID,
T1.GROUP_TYPE_CDE,
T1.GROUP_TYPE_EFFECTIVE_DT,
T1.DISCOUNT_PCT,
T1.BILL_AT_ISSUE_IND,
T1.PLAN_TYPE_CDE,
T1.MAXIMUM_BENEFIT_PAID_IND,
T1.BUY_SELL_GROUP_TYPE_CDE,
T1.LONG_TERM_CARE_IND,
T1.DEFAULT_DIVIDEND_PCT,
T1.GUARANTEED_STANDARD_ISSUE_IND,
T1.EMPLOYER_PAID_PCT,
T1.EMPLOYER_PAID_DISCOUNT_PCT,
T1.PREEXISTING_CONDITION_LIMITATION_ID,
T1.DUE_DT_ALIGNMENT_IND,
T1.SERVICING_AGENCY_ID,
T1.SOURCE_MARKET_CDE,
T1.MARKET_CDE,
T1.MARKET_CDE_EFFECTIVE_DT,
T1.SOURCE_SALES_CATEGORY_CDE,
T1.SALES_CATEGORY_CDE,
T1.SALES_CATEGORY_EFFECTIVE_DT,
T1.DEFAULT_DIVIDEND_PCT_EFFECTIVE_DT,
T1.UNDERWRITING_PROCESSOR_ID,
T1.PARENT_GROUP_NR_ID,
T1.ERISA_PLAN_CDE,
T1.ERISA_PLAN_EFFECTIVE_DT,
T1.EMPLOYER_PAID_EFFECTIVE_DT,
T1.EMPLOYER_PAID_DISCOUNT_EFFECTIVE_DT,
T1.SALARY_DEDUCTION_IND,
T1.MGI_IND,
T1.ENDR_IND,
T1.ENDR_DT,
T1.SERVICING_AGENT_ID,
T1.SOURCE_BILLING_FREQUENCY_CDE,
T1.BILLING_FREQUENCY_CDE,
T1.SOURCE_BILL_TYPE_CDE,
T1.BILLING_TYPE_CDE,
T1.LEVEL_PCT,
T1.CROSSOVER_YEAR_TXT,
T1.EMPLOYEE_RECEIVE_DIVIDEND_IND,
T1.EMPLOYEE_RECEIVE_PREMIUM_IND,
T1.SIC_CDE,
T1.GROUP_CLASS_CDE,
T1.BEGIN_BILLING_DT,
T1.SECOND_DUE_DAY_TXT,
T1.SOURCE_NAME
FROM VT_PRE_WORK_TYPE2 T1
LEFT JOIN VT_PRE_WORK_TYPE2 T2
ON T1.DIM_NON_MASTERED_PARTY_NATURAL_KEY_HASH_UUID = T2.DIM_NON_MASTERED_PARTY_NATURAL_KEY_HASH_UUID
AND T1.RW_NUM = T2.RW_NUM; 
