/*
    FileName: party_cdamprlife_dim_party.sql
    Author: MM14294
    SUBJECT AREA : Party
    SOURCE: CDA_MPRLIFE
    Teradata Source Code: 41
    ...
    ---------------------------------------------------------------------------------------------------------------
    JIRA 3289          Party-Tier2             07/05          First Version of DDL for Tier-2
    ------------------------------------------------------------------------------------------------------------------
*/

TRUNCATE TABLE EDW_STAGING.PARTY_CDAMPRLIFE_DIM_PARTY_PRE_WORK;

TRUNCATE TABLE EDW_WORK.PARTY_CDAMPRLIFE_DIM_PARTY;

INSERT INTO  EDW_STAGING.PARTY_CDAMPRLIFE_DIM_PARTY_PRE_WORK
(
...
(
	...
	ROW_NUMBER() OVER(PARTITION BY BENE_ROW_ADM_SYS_NAME,BENE_ROW_CTRT_PREFIX,BENE_ROW_CTRT_NO,BENE_ROW_CTRT_SUFFIX,BENE_ROW_CNTR ORDER BY BENEFICIARY_EFFECTIVE_DT DESC) AS RNK
	FROM
	(
		SELECT 
		...
		FROM EDW_STAGING.CDA_MPR_LIFE_EDW_BENE_DELTA
	)SOURCE_DATASET
...
INSERT INTO  EDW_WORK.PARTY_CDAMPRLIFE_DIM_PARTY
(
    DIM_PARTY_NATURAL_KEY_HASH_UUID
   ,PARTY_ID
   ,FIRST_NM
...
FROM EDW_STAGING.PARTY_CDAMPRLIFE_DIM_PARTY_PRE_WORK SRC
...
(
    ...
    (	
     ...
     FROM EDW_STAGING.PARTY_CDAMPRLIFE_DIM_PARTY_PRE_WORK S
