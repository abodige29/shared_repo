/*
    ...
    ---------------------------------------------------------------------------------------------------------------
    TERSUN-3525             Party-Tier2    09/02                Initial version      
    ------------------------------------------------------------------------------------------------------------------
*/
...
(
    SELECT 
     CASE WHEN (BEN_FRST_NM, BEN_MDL_NM, BEN_LST_NM, CARR_ADMIN_SYS_CD, HLDG_KEY_PFX, HLDG_KEY, HLDG_KEY_SFX, BEN_ARGMT_TXT, BEN_ROW_CNTR_CD) IS NULL 
    	THEN UUID_GEN(NULL)::UUID 
    	ELSE UUID_GEN(BEN_FRST_NM, BEN_MDL_NM, BEN_LST_NM, CARR_ADMIN_SYS_CD, HLDG_KEY_PFX, HLDG_KEY, HLDG_KEY_SFX, BEN_ARGMT_TXT, BEN_ROW_CNTR_CD)::UUID 
    ...
    ,COALESCE(BEN_FRST_NM,'')||COALESCE(BEN_MDL_NM,'')||COALESCE(BEN_LST_NM,'')||
     COALESCE(CARR_ADMIN_SYS_CD,'')||COALESCE(HLDG_KEY_PFX,'')||COALESCE(HLDG_KEY,'')||COALESCE(HLDG_KEY_SFX,'')||
     COALESCE(SUBSTRING(BEN_ARGMT_TXT,1,100),'')||COALESCE(BEN_ROW_CNTR_CD,'') AS PARTY_ID
    ,COALESCE(BEN_FRST_NM,'')||COALESCE(BEN_MDL_NM,'')||COALESCE(BEN_LST_NM,'')||
     COALESCE(CARR_ADMIN_SYS_CD,'')||COALESCE(HLDG_KEY_PFX,'')||COALESCE(HLDG_KEY,'')||COALESCE(HLDG_KEY_SFX,'')||
     COALESCE(SUBSTRING(BEN_ARGMT_TXT,1,100),'')||COALESCE(BEN_ROW_CNTR_CD,'') AS PARTY_PRIOR_ID  
    ,SOR_PARTY_ID
    ,PARTY_ID_TYPE_CDE 
    ,BEGIN_DT  
    ,BEGIN_DTM  
    ...
    BEN_DATA_TO_DT                                                AS BEN_DATA_TO_DT
...
/* insert beneficiary records into work from tersun source view */
...
/* delete VNTG1 records  from party_master */

DELETE FROM EDW.PARTY_MASTER_OF_MASTERS_XREF WHERE SOURCE_SYSTEM_ID IN ('241','38');
...
SELECT
    ...
    ,UPDATE_AUDIT_ID   
    ,SOURCE_DELETE_IND  
FROM EDW_WORK.PARTY_CDAVNTG1LIFE_PARTY_MASTER_OF_MASTERS_XREF;
