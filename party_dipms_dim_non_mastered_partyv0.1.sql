...
VOLTAGEACCESSDATE(DOB,'dob_date') AS BIRTH_DT,
...
CASE WHEN A.RW_NUM<B.RW_NUM AND A.RW_NUM=B.RW_NUM-1 AND A.END_DT>B.BEGIN_DT- INTERVAL '1' SECOND THEN 
B.BEGIN_DT-INTERVAL '1' SECOND ELSE A.END_DTM END AS END_DTM,
A.SOURCE_SYSTEM_ID,
...
FROM(
SELECT 
VOLTAGEPROTECT(CLEAN_STRING(VOLTAGEACCESS(SRC.FRST_NM,'name')),'name') AS FIRST_NM,
VOLTAGEPROTECT(CLEAN_STRING(VOLTAGEACCESS(SRC.LST_NM,'name')),'name') AS LAST_NM,
...
CLEAN_STRING(APPL.HLDG_KEY) || SRC.BENE_ID || COALESCE (FIRST_NM,'')|| COALESCE (LAST_NM,'')  AS PARTY_ID
FROM 
PROD_NBR_VW_TERSUN.NB_PRTY_BENE_VW SRC 
LEFT JOIN 
(SELECT DISTINCT HLDG_KEY,APPL_ID FROM PROD_NBR_VW_TERSUN.NB_APPL_VW APPL )APPL 
...
NULL AS MARITAL_STATUS_CDE,
SOURCE_LOGIN_ID,
'00001/01/01'::DATE AS BEGIN_DT,
'00001/01/01'::TIMESTAMP(6) AS BEGIN_DTM,
CURRENT_TIMESTAMP(6) AS  ROW_PROCESS_DTM,
:audit_id AS AUDIT_ID,
FALSE AS LOGICAL_DELETE_IND,
TRUE AS CURRENT_ROW_IND,
...
VOLTAGEPROTECT(CLEAN_STRING(VOLTAGEACCESS(FRST_NM,'name')),'name') AS FIRST_NM,
VOLTAGEPROTECT(CLEAN_STRING(VOLTAGEACCESS(LST_NM,'name')),'name') AS LAST_NM,
VOLTAGEPROTECT(CLEAN_STRING(VOLTAGEACCESS(LOG_IN_ID,'account_char')),'account_char') AS SOURCE_LOGIN_ID,
...
DROP TABLE IF EXISTS TEMP_OWN_VW2;

CREATE LOCAL TEMPORARY TABLE TEMP_OWN_VW2 ON COMMIT PRESERVE ROWS AS 
SELECT *,ROW_NUMBER() OVER (PARTITION BY DIM_NON_MASTERED_PARTY_NATURAL_KEY_HASH_UUID ORDER BY BEGIN_DT,END_DT)
AS RW_NUM FROM TEMP_OWN_VW1 ORDER BY DIM_NON_MASTERED_PARTY_NATURAL_KEY_HASH_UUID;

DROP TABLE IF EXISTS TEMP_OWN_VW;

CREATE LOCAL TEMPORARY TABLE TEMP_OWN_VW ON COMMIT PRESERVE ROWS AS 
SELECT * FROM  EDW_WORK.PARTY_WINRISK_DIM_NON_MASTERED_PARTY  WHERE 1<>1;

INSERT /*DIRECT*/ INTO TEMP_OWN_VW(
DIM_NON_MASTERED_PARTY_NATURAL_KEY_HASH_UUID,
PARTY_ID,
FIRST_NM,
MIDDLE_NM,
