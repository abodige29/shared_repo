...
/*
    ...
    FileName: party_cdaopmlife_rel_party_agreement.sql
    Author: mm15263
    SUBJECT AREA : Party
    SOURCE: BENE_DATA_VW
    Teradata Source Code: 40
...
DROP TABLE IF EXISTS TEMP_BENE_DATA_VW_OPM;

/* LOAD CLEANSED DATA IN TEMP TABLE */

CREATE LOCAL TEMPORARY TABLE TEMP_BENE_DATA_VW_OPM ON COMMIT PRESERVE ROWS AS 
SELECT 
...
UUID_GEN(CARR_ADMIN_SYS_CD,HLDG_KEY_PFX,HLDG_KEY,HLDG_KEY_SFX,BEN_ROW_CNTR_CD)::UUID  AS DIM_PARTY_NATURAL_KEY_HASH_UUID,
...
TRUNCATE TABLE EDW_WORK.PARTY_CDAOPMLIFE_REL_PARTY_AGREEMENT;		/* Work table for OPM Beneficiary REL_PARTY_AGREEMENT */

/* INSERTING DATA IN WORK TABLE AND CALCULATE END DATES FOR INCOMING DATA */


INSERT INTO EDW_WORK.PARTY_CDAOPMLIFE_REL_PARTY_AGREEMENT 
(
DIM_AGREEMENT_NATURAL_KEY_HASH_UUID,
DIM_PARTY_NATURAL_KEY_HASH_UUID,
REF_PARTY_ROLE_NATURAL_KEY_HASH_UUID,
...
SELECT ANALYZE_STATISTICS('EDW_WORK.PARTY_CDAOPMLIFE_REL_PARTY_AGREEMENT');


DELETE FROM EDW.REL_PARTY_AGREEMENT WHERE SOURCE_SYSTEM_ID IN ('231','40') AND SOURCE_BENEFICIARY_ROLE_CDE='Bene';
...
BENEFICIARY_UNFORMATTED_NM
FROM
EDW_WORK.PARTY_CDAOPMLIFE_REL_PARTY_AGREEMENT;
