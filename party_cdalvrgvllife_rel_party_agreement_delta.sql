...
TRUNCATE TABLE EDW_WORK.PARTY_CDALVRGVLLIFE_REL_PARTY_AGREEMENT;


---------- GENERATING UUID FOR CONTRACT AGREEMENTS TO JOIN WITH BENE TABLE ------------

DROP TABLE IF EXISTS TEMP_LVRGVL_CTRT_DIM_AGREEMENT_TBL ;
CREATE LOCAL TEMPORARY TABLE TEMP_LVRGVL_CTRT_DIM_AGREEMENT_TBL ON COMMIT PRESERVE ROWS AS
/*+DIRECT*/
SELECT 
...
FROM (			
		...
		FROM EXT_EDAP_STAGING.CDA_LVRGVL_EDW_CONTRACT_FULL SRC1   
		...
		SELECT DISTINCT
						...
						LPAD(CLEAN_STRING(ctrt_row_ctrt_no),20,'0')		AS CTRT_ROW_CTRT_NO,
						UDF_ISNUM_LPAD(CLEAN_STRING(ctrt_row_ctrt_suffix), 20, '0', TRUE)		AS CTRT_ROW_CTRT_SUFFIX
		FROM EXT_EDAP_STAGING.CDA_LVRGVL_EDW_CONTRACT_DELTA SRC2
		WHERE PROCESS_IND='D'
	  ) A ;
...
/*+DIRECT*/
SELECT *--,CLEAN_STRING(VOLTAGEACCESS(AGRMNT_NUM,'ssn_char')) AS AGRMNT_NUM_DE,CLEAN_STRING(VOLTAGEACCESS(MEMBER_ID,'sorparty')) AS MEMBER_ID_DE
...
INSERT INTO EDW_STAGING.PARTY_CDALVRGVLLIFE_REL_PARTY_AGREEMENT_PRE_WORK
(
	DIM_AGREEMENT_NATURAL_KEY_HASH_UUID,
	DIM_PARTY_NATURAL_KEY_HASH_UUID,
	REF_PARTY_ROLE_NATURAL_KEY_HASH_UUID,
...
INNER JOIN TEMP_LVRGVL_CTRT_DIM_AGREEMENT_TBL CTRT ON CTRT.DIM_AGREEMENT_NATURAL_KEY_HASH_UUID=DEDUP.DIM_AGREEMENT_NATURAL_KEY_HASH_UUID   
...
/*INSERT SCRIPT FOR PRE WORK TABLE - END/DELETES */
...
(
	...
	SELECT 
		...
		and bene_name_last 					is null
		and BENE_NAME_SUFFIX 				is null
		and BENE_NAME_PREFIX 				is null then NULL
		when UPPER(BENEFICIARY_NAME_FORMAT_CDE) = 'U'
		and (bene_name_first 				is not null
		or bene_name_middle 				is not null
		or bene_name_last 					is not null
	...
	FROM
		...
		CURRENT_TIMESTAMP::date		AS END_DT,
		CURRENT_TIMESTAMP::timestamp(6)		AS END_DTM,
		'85'		AS SOURCE_SYSTEM_ID, 
		FALSE		AS RESTRICTED_ROW_IND,
		:audit_id		AS UPDATE_AUDIT_ID,
		CLEAN_STRING(bene_relationship)		AS BENEFICIARY_RELATIONSHIP_NM,
...
AND (
	...
	(
		...
		REF_PARTY_ROLE_NATURAL_KEY_HASH_UUID,
		REF_PARTY_SUB_ROLE_NATURAL_KEY_HASH_UUID
		FROM EDW_STAGING.PARTY_CDALVRGVLLIFE_REL_PARTY_AGREEMENT_PRE_WORK
	);
...
FROM EDW_STAGING.PARTY_CDALVRGVLLIFE_REL_PARTY_AGREEMENT_PRE_WORK SRC
