...
SELECT 
	...
	ROW_NUMBER() OVER(PARTITION BY POLICY_NUMBER,CASE_ID,LAST_NAME,FIRST_NAME,LOGINID,ROLE_CD,CHANGE_MODE 
    ORDER BY LST_UPDT_DT DESC) AS RNK
    FROM(
	SELECT 
		appl.POLICY_NUMBER AS POLICY_NUMBER,
		appl.CASE_ID       AS CASE_ID,
		...
		req.CASE_OWN_FR_DT AS LST_UPDT_DT 
		FROM PROD_NBR_VW_TERSUN.NB_PRTY_CASE_OWN_VW req
		LEFT JOIN 
		APPL_VW appl
		on req.APPL_ID=req.APPL_ID
	)Q_1
	)Q_2
WHERE RNK=1;
